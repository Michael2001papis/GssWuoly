...
- `index.html` בשורש משתמש גם ב־meta refresh וגם ב־location.replace (כפילות אסטרטגיה).
- יש קבצים שאינם בשימוש: `pages/login/js/email-config.js` ו־`pages/login/vendor/email.min.js` (מומלץ להסיר לפני הגשה).
...
- [ ] להסיר קבצי EmailJS לא בשימוש (אם באמת לא משתמשים)
